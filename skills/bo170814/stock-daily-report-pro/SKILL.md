...
```bash
# 使用 clawhub 安装
clawhub install stock-daily-report-pro
```
...
```bash
25 9 * * 1-5 cd /path/to/stock-daily-report-pro && python3 generate_report.py --format both
```
...
添加（交易日 9:25 生成并推送）：
```bash
25 9 * * 1-5 cd /path/to/stock-daily-report-pro && python3 schedule_push.py
```
